...
$(document).ready(function () {
    ...
    $("#ImageCertificate").change(function () {
        var memid = $("#tbl_PersonalDetails option:selected").val();
        
        if (memid == "" || memid == "Select") {
            alert("Please Select MemberId");
            $("#ImageCertificate").val(null);
            return false;
        }
        ...
        $.ajax({
            url: "../SecularEducation/UploadFile",
            type: "POST",
            processData: false,
            contentType: false,
            data: data,
    ...
    $.getJSON("GetAll",
        function (json) {
            ...
            for (var i = 0; i < json.length; i++) {
                ...
                tr.append('<td style=display:none;><a href="ViewProfile"/' + json[i].SecularId + '>' + json[i].MemberID + '</a></td>');
                //TODO Rajesh
                ...
                tr.append("<td> <a href=/Images/SecularCertificate/" + json[i].Certificate + " view>View</a></td>");
                $('table').append(tr);
