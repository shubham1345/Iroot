...
$(document).ready(function () {
    ...
    $('#printbtn').click(function () {
        window.open('../SpiritualCommunityDoc/SpiritualCommunityDoc1Report');
    });
    ...
    $.getJSON("GetAll",
        function (json) {
            ...
            for (var i = 0; i < json.length; i++) {
                ...
                //tr.append('<td><a href=ViewProfile/' + json[i].SpiritualCommunityId + '>' + json[i].SpiritualCommunityId + '</a></td>');
                tr.append("<td>" + json[i].DoccumentName + "</td>");
                tr.append("<td>" + json[i].SubDoccument + "</td>");
                tr.append("<td>" + json[i].Title + "</td>");
                tr.append("<td>" + json[i].Date + "</td>");
                tr.append("<td> <a href=/Images/SpiritualCommunityDoc/" + json[i].File + " download>Download</a></td>");
                tr.append("<td> <a href=/Images/SpiritualCommunityDoc/" + json[i].File + " view>View</a></td>");
                tr.append('<td><a href="#" onclick="return getbyID(' + json[i].SpiritualCommunityId + ')">Edit</a> | <a href="#" onclick="Delele(' + json[i].SpiritualCommunityId + ')">Delete</a></td>')
                $('table').append(tr);

            }
            $('SpiritualCommunityDocInfo').DataTable();
        });
}

function loadDataListItems() {
    $.ajax({
        url: "../CommonDataListItems/GetDatalistItemsByModuleName/?DlistName=Spiritual and Community Life",
        type: "GET",
...
function Add() {
    ...
    var general =
        {
            SpiritualCommunityId: $('#SpiritualCommunityId').val(),
...
function getbyID(GId1) {
    $.ajax({
        url: "../SpiritualCommunityDoc/GetById/" + GId1,
        type: "GET",
        contentType: "application/json;charset=UTF-8",
        dataType: "json",
        success: function (result) {
            
            $('#SpiritualCommunityId').val(result.SpiritualCommunityId);
            // $('#tbl_FomDoc option:selected').val(result.SpiritualCommunityId)
...
function Update() {
    ...
    $.ajax({
        url: "../SpiritualCommunityDoc/Update",
...
function Delele(ID) {
    ...
    if (ans) {
        $.ajax({
            url: "../SpiritualCommunityDoc/Delete/" + ID,
            type: "POST",
            contentType: "application/json;charset=UTF-8",
            dataType: "json",
            success: function (result) {
...
$("#FileUpload").change(function () {
    ...
    $.ajax({
        url: "/SpiritualCommunityDoc/UploadFile",
        type: "POST",
        processData: false,
        contentType: false,
        data: data,
...
function clearTextBox() {
    ...
    $('#SpiritualCommunityId').css('border-color', 'lightgrey');
...
function validate() {
    var isValid = true;
    if ($('#SpiritualCommunityId').val().trim() == "") {
        $('#SpiritualCommunityId').css('border-color', 'Red'); isValid = false;
    }
    else {
        $('#SpiritualCommunityId').css('border-color', 'lightgrey');
    }
