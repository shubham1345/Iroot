...
$(document).ready(function () {
    ...
    $('#printbtn').click(function () {
        window.open('../ProvincialDoc/ProvincialDoc1Report');
    });
    ...
    $.getJSON("GetAll",
        function (json) {
            ...
            for (var i = 0; i < json.length; i++) {
                ...
                tr = $('<tr />');
                tr.append("<td hidden='hidden'>" + json[i].ProvincialId + "</td>");
                //tr.append('<td><a href=ViewProfile/' + json[i].ProvincialId + '>' + json[i].ProvincialId + '</a></td>');
                tr.append("<td>" + json[i].DoccumentName + "</td>");
                tr.append("<td>" + json[i].Title + "</td>");
                tr.append("<td>" + json[i].Date + "</td>");
                tr.append("<td> <a href=/Images/ProvincialDoc/" + json[i].File + " download>Download</a></td>");
                tr.append("<td> <a href=/Images/ProvincialDoc/" + json[i].File + " view>View</a></td>");
                tr.append('<td><a href="#" onclick="return getbyID(' + json[i].ProvincialId + ')">Edit</a> | <a href="#" onclick="Delele(' + json[i].ProvincialId + ')">Delete</a></td>')
                $('table').append(tr);

            }
            $('ProvincialDocInfo').DataTable();
        });
}
...
function Add() {
    ...
    var general =
        {
            ProvincialId: $('#ProvincialId').val(),
            // DoccumentName: $('#DoccumentName').val(),
            // DoccumentName: $("#tbl_ProvincialDoc option:selected").text(),
            DoccumentName: $('#DoccumentName').val(),
            Title: $('#Title').val(),
            Date: $('#Date').val(),
            File: $('#File').val()
...
function getbyID(GId1) {
    $.ajax({
        url: "../ProvincialDoc/GetById/" + GId1,
        type: "GET",
        contentType: "application/json;charset=UTF-8",
        dataType: "json",
        success: function (result) {
            
            $('#ProvincialId').val(result.ProvincialId);
            // $('#tbl_ProvincialDoc option:selected').val(result.ProvincialId)
    ...
    var empObj =
        {
            ProvincialId: $('#ProvincialId').val(),
            //  DoccumentName: $('#DoccumentName').val(),
            // ProvincialId: $("#tbl_FomDoc option:selected").text(),
            DoccumentName: $('#DoccumentName').val(),
            Title: $('#Title').val(),
            Date: $('#Date').val(),
            File: $('#File').val(),
...
function clearTextBox() {
    ...
    $('#ProvincialId').css('border-color', 'lightgrey');
...
function validate() {
    var isValid = true;
    if ($('#ProvincialId').val().trim() == "") {
        $('#ProvincialId').css('border-color', 'Red'); isValid = false;
    }
    else {
        $('#ProvincialId').css('border-color', 'lightgrey');
    }
