...
$(document).ready(function () {
    ...
    $('#printbtn').click(function () {
        window.open('../MinistryDoc/MinistryDoc1Report');
    });
    ...
    $.getJSON("GetAll",
        function (json) {
            ...
            for (var i = 0; i < json.length; i++) {
                ...
                tr.append("<td> <a href=/Images/MinistryDoc/" + json[i].File + " download>Download</a></td>");
                tr.append("<td> <a href=/Images/MinistryDoc/" + json[i].File + " view>View</a></td>");
                tr.append("<td> <a href=/Images/MinistryDoc1/" + json[i].Photo + " download>download</a></td>");
                tr.append('<td><a href="#" onclick="return getbyID(' + json[i].MinistryDocId + ')">Edit</a> | <a href="#" onclick="Delele(' + json[i].MinistryDocId + ')">Delete</a></td>')
                $('table').append(tr);
...
function Add() {
    ...
    var general =
        {
            MinistryDocId: $('#MinistryDocId').val(),
...
function getbyID(GId1) {
    $.ajax({
        url: "../MinistryDoc/GetById/" + GId1,
        type: "GET",
        contentType: "application/json;charset=UTF-8",
        dataType: "json",
        success: function (result) {
            
            $('#MinistryDocId').val(result.MinistryDocId);
...
function clearTextBox() {
    ...
    $('#MinistryDocId').css('border-color', 'lightgrey');
...
function validate() {
    var isValid = true;
    if ($('#MinistryDocId').val().trim() == "") {
        $('#MinistryDocId').css('border-color', 'Red'); isValid = false;
    }
    else {
        $('#MinistryDocId').css('border-color', 'lightgrey');
    }
